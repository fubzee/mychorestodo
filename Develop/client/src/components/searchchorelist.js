...
const Choreslist = () => { 
      ...
      const [getParentChores, { error:p_error, data: p_data }] = useMutation(QUERY_ALL_PARENT_CHORES);
      const [getChildChores, { error:c_error, data: c_data }]  = useMutation(QUERY_ALL_CHILDREN_CHORES);
      let results = " ";
      ...
      if (data.usertype === "Parent") {  
              ...
              try { const { Parentchores } = getParentChores({ variables: { parentId : Parent._id}})
            ...
            console.log("Child Case", Child)
               try { const { childchores } =  getChildChores({ variables: { childId : Child._id}})
              console.log(c_data)
